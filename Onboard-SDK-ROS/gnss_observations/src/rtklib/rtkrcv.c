...
#define NAVIFILE    "data/rtkrcv.nav"        /* navigation save file */
#define STATFILE    "data/rtkrcv_%Y%m%d%h%M.stat"  /* solution status file */
#define TRACEFILE   "data/rtkrcv.trace" /* debug trace file */
...
static char ws_path_[MAXSTR];
static char opt_file_[MAXSTR];
...
vt_t vt={0};
int outstat=0,tracel=2;

prcopt_t* prcopt_exchange;
...
*     
*-----------------------------------------------------------------------------*/
extern int rtkrcv_start(const char* ws_path, const char* opt_file)
{
    int i,start=1,port=0;
    char *dev="",file[MAXSTR]="";
    strcpy(ws_path_, ws_path);
    strcpy(opt_file_, opt_file);
    /*sysopts->isposdebug = 0;*/
    
    if (tracel>0) {
        char trace_file[MAXSTR];
        sprintf(trace_file, "%s/%s", ws_path_, TRACEFILE);
    ...
    /* load options file */
    if (!*file) sprintf(file,"%s",opt_file_);
    printf("%s\r\n", file);
    ...
    char nav_file[MAXSTR];
    sprintf(nav_file, "%s/%s", ws_path_, NAVIFILE);
    if (!readnav(nav_file,&svr.nav)) {
        fprintf(stderr,"no navigation data: %s\n",nav_file);
    }
    if (outstat>0) {
        char stat_file[MAXSTR];
        sprintf(stat_file, "%s/%s", ws_path_, STATFILE);
        rtkopenstat(stat_file,outstat);
    }
    /* open monitor port */
    ...
    /* start rtk server */
    if (!startsvr(&vt)) return -1;
    
    return 0;
}

extern int rtkrcv_stop() {
    ...
    if (tracel>0) traceclose();
    
    char nav_file[MAXSTR];
    sprintf(nav_file, "%s/%s", ws_path_, NAVIFILE);
    /* save navigation data */
    if (!savenav(nav_file,&svr.nav)) {
