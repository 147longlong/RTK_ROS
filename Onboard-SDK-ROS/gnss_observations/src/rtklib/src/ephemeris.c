...
#define MAXECORSSR 100.0           /* max orbit correction of ssr (m) */
...
static eph_t *seleph(gtime_t time, int sat, int iode, const nav_t *nav)
{
    ...
    if (iode>=0||j<0) {
        trace(2,"no broadcast ephemeris: %s sys = %d sat=%2d iode=%3d\n",time_str(time,0),
              satsys(sat,NULL),sat,iode);
        return NULL;
    }
    return nav->eph+j;
}
                      ...
                      int opt, double *rs, double *dts, double *var, int *svh)
{
    ...
    if (!ssr->t0[0].time) {
        trace(2,"no ssr orbit correction: %s sys = %2d sat=%2d\n",time_str(time,0),satsys(sat,NULL),sat);
        return 0;
    }
    if (!ssr->t0[1].time) {
